...
class MaximaController:
    ...
    def ejecutar_algoritmo(self):
        """Genera árbol de expansión MÁXIMA usando Kruskal (sin ciclos)"""
        if self.vertices < 2:
            DialogoClave(0, "Error", "mensaje", self.vista,
        ...
        # Ordenar por peso (de MAYOR a MENOR para árbol MÁXIMO)
        ...
        for peso, arista in aristas_con_peso:
            ...
            # Intentar agregar la arista solo si NO forma ciclo
            if uf.unir(u, v):
        ...
        # Verificar si el grafo era conexo
        if len(self.arbol_aristas) < self.vertices - 1:
            DialogoClave(0, "Advertencia", "mensaje", self.vista,
                         f"El grafo no es conexo.\n"
                         f"Se generó un bosque con {len(self.arbol_aristas)} aristas\n"
                         f"en lugar de {self.vertices - 1} aristas.").exec()
    ...
    def mostrar_resultados(self):
        ...
        info_html = f"""
        <h3>Árbol de Expansión Máxima</h3>
            ...
            for idx_circuito, cuerda in enumerate(cuerdas, 1):
                ...
                if camino:
                    ...
                    for arista in todas_aristas:
                        if arista in aristas_en_circuito:
                            peso = self.ponderaciones.get(arista, self.ponderaciones.get((arista[1], arista[0]), 1))
                            matrices_html += f"<td align='center' style='padding: 10px; font-weight: bold;'>{peso}</td>"
                        else:
                            matrices_html += "<td align='center' style='padding: 10px;'>-</td>"
                else:
                    for _ in todas_aristas:
                        matrices_html += "<td align='center' style='padding: 10px;'>-</td>"
    ...
    def limpiar_resultado(self):
        """Limpia los resultados del árbol"""
        self.arbol_aristas = []
        self.peso_total = 0
        self.vista.visual_arbol.set_grafo(0, [], {}, {})
        self.vista.info_text.setHtml("")
        self.vista.circuitos_text.setHtml("")
        self.vista.circuitos_fund_text.setHtml("")
        self.vista.conjuntos_text.setHtml("")
        self.vista.matrices_text.setHtml("")
