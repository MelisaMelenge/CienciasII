...
class CambioBase(QMainWindow):
    def __init__(self, cambiar_ventana):
        super().__init__()
        self.cambiar_ventana = cambiar_ventana
        self.controller = CambioBaseController()
        self.bloques = []
        self.num_claves = 0
        self.tamanio_bloque = 0

        self.setWindowTitle("Ciencias de la Computación II - Cambio de Base (Externa)")
        ...
        titulo = QLabel("Ciencias de la Computación II - Cambio de base (Externa)")
        ...
        for lbl in (lbl_claves, lbl_digitos, lbl_base):
        ...
        self.base_input.setRange(2, 16)
        self.base_input.setValue(9)
        self.base_input.setFixedWidth(100)
        ...
        fila_controles.addWidget(lbl_base)
        fila_controles.addWidget(self.base_input)
    ...
    def crear_estructura(self):
        ...
        try:
            ...
            datos = self.controller.crear_estructura(num_claves, base)
    ...
    def insertar_clave(self):
        ...
        # Caso: conflicto/colisión detectada
        if isinstance(res, tuple) and len(res) == 3 and res[1] == "collision":
            ...
            dlg_confirm = DialogoClave(0, "Colisión detectada", "confirmar", self, detalle)
            if dlg_confirm.exec():
                ok, msg = self.controller.insertar_en_zona_colisiones(info['clave'])
                DialogoClave(0, "Zona de colisiones", "mensaje", self, msg).exec()

        else:
            # Insert normal o error
            ok, msg = res
            if ok:
                DialogoClave(0, "Insertar clave", "mensaje", self, msg).exec()
            else:
                DialogoClave(0, "Insertar clave", "mensaje", self, msg).exec()
    ...
    def eliminar_clave(self):
        ...
        if dlg.exec():
            ...
            if not ok:
                DialogoClave(0, "Eliminar", "mensaje", self, msg).exec()
            self.bloques = self.controller.bloques
            self.actualizar_visualizacion()

    def guardar_estructura(self):
        ruta, _ = QFileDialog.getSaveFileName(self, "Guardar estructura", "cambio_base_externa.json", "JSON (*.json)")
        if ruta:
            try:
                self.controller.guardar_estructura(ruta)
                DialogoClave(0, "Guardar", "mensaje", self,
                             "Estructura guardada correctamente.").exec()
            except Exception as e:
                DialogoClave(0, "Error", "mensaje", self,
                             f"No se pudo guardar: {e}").exec()
    ...
    def cargar_estructura(self):
        ...
        if ruta:
            try:
                ...
                self.tamanio_bloque = datos['tamanio_bloque']
                self.actualizar_visualizacion()
                DialogoClave(0, "Cargar", "mensaje", self,
                             "Estructura cargada correctamente.").exec()
            except Exception as e:
                DialogoClave(0, "Error", "mensaje", self,
                             f"No se pudo cargar: {e}").exec()
    ...
    def actualizar_visualizacion(self):
        ...
        # --- Zona de Colisiones ---
        if hasattr(self.controller, 'zona_colisiones') and self.controller.zona_colisiones.zona:
            ...
            zona_layout.setAlignment(Qt.AlignLeft)
    ...
    def _resaltar(self, bloque_idx, offset):
        """Resalta una celda encontrada"""
        self.actualizar_visualizacion()
        bloques_container = self.contenedor_layout.itemAt(1).widget()
