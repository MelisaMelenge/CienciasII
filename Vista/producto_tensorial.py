...
class ProductoTensorial(QMainWindow):
    def __init__(self, cambiar_ventana):
        ...
        self.setWindowTitle("Ciencias de la Computación II - Producto Tensorial")
        ...
        titulo = QLabel("Ciencias de la Computación II - Producto Tensorial de Grafos")
        ...
        label_transformacion = QLabel("Generar Producto")
        ...
        self.btn_generar = QPushButton("⊗ Producto Tensorial")
        ...
        self.visual_producto = VisualizadorGrafo("Producto Tensorial G₁ ⊗ G₂", parent=self, es_editable=False)
        ...
        for visual in (self.visual_g1, self.visual_g2, self.visual_producto):
        ...
        # ======= CONEXIÓN PRODUCTO =======
        self.btn_generar.clicked.connect(self.generar_producto_tensorial)
    ...
    # ==================== PRODUCTO TENSORIAL ====================
    def generar_producto_tensorial(self):
        ...
        # PRODUCTO TENSORIAL (también llamado producto directo o categórico)
        # V(G₁ ⊗ G₂) = V(G₁) × V(G₂)
        # Dos vértices (g₁,h₁) y (g₂,h₂) son adyacentes en G₁ ⊗ G₂ si y solo si:
        # g₁ es adyacente a g₂ en G₁ Y h₁ es adyacente a h₂ en G₂
        ...
        for i in range(self.grafo1_vertices):
            for j in range(self.grafo2_vertices):
                ...
                etiquetas_producto[indice] = f"({etiq_g1},{etiq_g2})"
                indice += 1
        ...
        aristas_vistas = set()
        for idx1 in range(vertices_producto):
            g1, h1 = indice_a_par[idx1]
            for idx2 in range(idx1 + 1, vertices_producto):
                g2, h2 = indice_a_par[idx2]

                # Verificar si g1 es adyacente a g2 en G₁ Y h1 es adyacente a h2 en G₂
                g_arista = (min(g1, g2), max(g1, g2))
                h_arista = (min(h1, h2), max(h1, h2))

                if g_arista in adyacentes_g1 and h_arista in adyacentes_g2:
                    arista_producto = (min(idx1, idx2), max(idx1, idx2))
                    if arista_producto not in aristas_vistas:
                        aristas_producto.append(arista_producto)
                        aristas_vistas.add(arista_producto)

        ponderaciones_producto = {}

        self.visual_producto.set_grafo(
            vertices_producto,
            aristas_producto,
            etiquetas_producto,
            ponderaciones_producto
        ...
        DialogoClave(0, "Producto Generado", "mensaje", self,
                     f"Producto tensorial generado:\n"
                     f"Vértices: {vertices_producto}\n"
                     f"Aristas: {len(aristas_producto)}").exec()

    # ==================== OPERACIONES ADICIONALES ====================
    def guardar_grafo_producto(self):
        if not hasattr(self.visual_producto, 'vertices') or self.visual_producto.vertices == 0:
            DialogoClave(0, "Error", "mensaje", self,
                         "No hay producto tensorial para guardar.").exec()
            return
        archivo, _ = QFileDialog.getSaveFileName(
            self, "Guardar Producto Tensorial", "", "JSON Files (*.json)")
        if archivo:
