...
class UnionGrafos(QMainWindow):
    def __init__(self, cambiar_ventana):
        ...
        self.visual_union = VisualizadorGrafo("Unión (G1 ∪ G2)", parent=self, es_editable=False)
        ...
        btn_guardar_union.setStyleSheet("""
            QPushButton {
                background-color: #9c724a;
                color: #FFEAC5;
                padding: 8px 16px;
        ...
        btn_limpiar_union.setStyleSheet("""
            QPushButton {
                background-color: #9c724a;
                color: #FFEAC5;
                padding: 8px 16px;
        ...
        botones_extras.addWidget(btn_guardar_union)
        botones_extras.addWidget(btn_limpiar_union)
    ...
    def calcular_union(self):
        ...
        # Vértices de la unión: todas las etiquetas de ambos grafos
        etiquetas_union = etiquetas_g1.union(etiquetas_g2)

        if not etiquetas_union:
            DialogoClave(0, "Unión vacía", "mensaje", self,
                         "No hay vértices en ninguno de los grafos.\n\n"
                         "La unión está vacía.").exec()
        ...
        # Crear nuevo mapeo para la unión
        etiquetas_union_dict = {}
        etiq_a_nuevo_indice = {}
        nuevo_indice = 0
        for etiq in sorted(etiquetas_union):
            etiquetas_union_dict[nuevo_indice] = etiq
            etiq_a_nuevo_indice[etiq] = nuevo_indice
            nuevo_indice += 1

        vertices_union = len(etiquetas_union)

        # Recolectar todas las aristas de ambos grafos (sin duplicados por etiquetas)
        aristas_union = []
        aristas_por_etiqueta = set()
        ...
        for arista1 in self.grafo1_aristas:
            ...
            if etiq_orig and etiq_dest:
                # Crear tupla ordenada de etiquetas para detectar duplicados
                arista_etiq = tuple(sorted([etiq_orig, etiq_dest]))
                if arista_etiq not in aristas_por_etiqueta:
                    aristas_por_etiqueta.add(arista_etiq)
                    # Agregar con nuevos índices
                    ...
                    aristas_union.append(nueva_arista)

        # Agregar aristas del Grafo 2
        for arista2 in self.grafo2_aristas:
            idx_orig_g2, idx_dest_g2 = arista2
            etiq_orig = self.grafo2_etiquetas.get(idx_orig_g2)
            etiq_dest = self.grafo2_etiquetas.get(idx_dest_g2)
        ...
        self.visual_union.set_grafo(vertices_union, aristas_union, etiquetas_union_dict)

        # Contar vértices exclusivos de cada grafo
        exclusivos_g1 = len(etiquetas_g1 - etiquetas_g2)
        exclusivos_g2 = len(etiquetas_g2 - etiquetas_g1)
        comunes = len(etiquetas_g1.intersection(etiquetas_g2))

        DialogoClave(0, "Unión calculada", "mensaje", self,
                     f" Unión calculada exitosamente:\n\n"
                     f" Total de vértices: {vertices_union}\n"
                     f"    • Del Grafo 1: {len(etiquetas_g1)}\n"
                     f"    • Del Grafo 2: {len(etiquetas_g2)}\n"
                     f"    • Exclusivos G1: {exclusivos_g1}\n"
                     f"    • Exclusivos G2: {exclusivos_g2}\n"
                     f"    • En común: {comunes}\n\n"
                     f" Total de aristas: {len(aristas_union)}\n"
                     f"    • De Grafo 1: {len(self.grafo1_aristas)}\n"
                     f"    • De Grafo 2: {len(self.grafo2_aristas)}\n\n"
                     f" La unión incluye TODOS los vértices\n"
                     f"y aristas de ambos grafos (sin duplicados).").exec()
    ...
    def guardar_union(self):
        if self.visual_union.num_vertices == 0:
            DialogoClave(0, "Error", "mensaje", self,
                         "Primero debes calcular la unión.").exec()
            return

        archivo, _ = QFileDialog.getSaveFileName(
            self, "Guardar Unión", "", "JSON Files (*.json)"
        )

        if archivo:
            try:
                datos = self.visual_union.get_datos_grafo()
                with open(archivo, 'w', encoding='utf-8') as f:
                    json.dump(datos, f, indent=4, ensure_ascii=False)
                DialogoClave(0, "Éxito", "mensaje", self,
                             f"Unión guardada exitosamente en:\n{archivo}").exec()
            except Exception as e:
                DialogoClave(0, "Error", "mensaje", self,
                             f"Error al guardar el archivo:\n{str(e)}").exec()
    ...
    def limpiar_todo(self):
        """Limpia todos los grafos y la unión"""
        self.grafo1_vertices = 0
        self.grafo1_aristas = []
        self.grafo1_etiquetas = {}
        self.vertices_g1.setValue(4)
    ...
    def limpiar_union(self):
        """Limpia solo la visualización de la unión"""
        if self.visual_union.num_vertices == 0:
            DialogoClave(0, "Información", "mensaje", self,
                         "La unión ya está vacía.").exec()
            return

        self.visual_union.set_grafo(0, [], {})
        DialogoClave(0, "Unión limpiada", "mensaje", self,
                     " La visualización de la unión\nha sido limpiada exitosamente.").exec()
